...
if __name__ == "__main__":
    ...
    if args.train: # train all models
        ...
        print('### Training Models with CNNs ###') # First type of network structure
        for i, (arch, kern) in enumerate(it.product(arch_setup, kernel_setup)):
            ...
            print(f'Architecture {arch} and Kernel {kern}: '
        ...
        torch.manual_seed(SEED)
        print('### Training Models with CNNs and Weight Sharing ###') # Second type of network structure
        for i, (arch, kern) in enumerate(it.product(arch_setup, kernel_setup)):
            kernel_conv = [kern] * (len(arch) - 1)
            fc_size = compute_fc_size(arch, kernel_conv)
            loss_train, loss_val, acc_val, acc_test, loss_test, model = repeat_train(
        ...
        print('### Training Models with CNNs, Weight Sharing and Auxiliary Losses ###') # Third type of network structure
        for i, (arch, kern) in enumerate(it.product(arch_setup, kernel_setup)):
            kernel_conv = [kern] * (len(arch) - 1)
            fc_size = compute_fc_size(arch, kernel_conv)
            loss_train, loss_val, acc_val, acc_test, loss_test, model = repeat_train(
        ...
        model.load_state_dict(torch.load('./models/cnn_ws_al_11.pkl'))

        torch.manual_seed(SEED)
        train_input, train_target, train_classes, test_input, test_target, test_classes = generate_pair_sets(N_SAMPLES)

        _, _, test_loader = generate_data_loaders_AL(train_input, train_target, train_classes,
                                                     test_input, test_target, test_classes, batch_size)
